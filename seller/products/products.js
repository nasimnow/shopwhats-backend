...
router.get("/count", (req, res) => {
  ...
  (select count(*) from catogories WHERE cat_user=${req.user.user.id}) as cat_count ,(select store_views from store_analytics WHERE user_id=${req.user.user.id} AND date="2021-01-03") as store_views,(select message_clicks from store_analytics WHERE user_id=${req.user.user.id}) as message_clicks`;
